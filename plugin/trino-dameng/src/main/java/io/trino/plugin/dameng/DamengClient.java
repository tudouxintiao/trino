...
public class DamengClient extends BaseJdbcClient {
    ...
    private static final int ZERO_PRECISION_TIME_COLUMN_SIZE = 13;
    ...
    public WriteMapping toWriteMapping(ConnectorSession session, Type type)
    {
        ...
        if (type instanceof TimeType timeType) {
            if (timeType.getPrecision() <= TimestampType.MAX_SHORT_PRECISION) {
                return WriteMapping.longMapping(format("time(%s)", timeType.getPrecision()), timeWriteFunction(timeType.getPrecision()));
            }
            return WriteMapping.longMapping(format("time(%s)", TimestampType.MAX_SHORT_PRECISION), timeWriteFunction(TimestampType.MAX_SHORT_PRECISION));
        }
        ...
        if (type instanceof TimestampType timestampType) {
            ...
            return WriteMapping.objectMapping(format("datetime(%s)", TimestampType.MAX_SHORT_PRECISION), longTimestampWriteFunction(timestampType, TimestampType.MAX_SHORT_PRECISION));
        }
